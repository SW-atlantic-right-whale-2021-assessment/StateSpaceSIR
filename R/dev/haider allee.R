pmsy_z_haider <- function(z,Pmsy, q = .02) {
    n = Pmsy * 100
    derivallee <- deriv(~0.1*n*(1-(n/100)^z)*((q*100)/(100-(q*100)) * (n/(q*100) - 1)), c("n","z","q"), function.arg = TRUE)
    check <- derivallee(n, z, q)
    with(attributes(check), gradient)[1]
}

sample.z <- NA
sample.z <- try(uniroot(pmsy_z_haider, Pmsy = .5, q = .2, lower= .4,upper=1)$root, silent = TRUE)
...
q = 0.2
curve(0.1*x*(1-(x/100))*((q*100)/(100-(q*100)) * (x/(q*100) - 1)), from = 0, to = 100)
